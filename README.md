...
Running `bun run dev:setup` (step 2) auto-generates `.env.local` with `CONVEX_DEPLOYMENT` and `NEXT_PUBLIC_CONVEX_URL`. Verify the file exists and contains:

```env
CONVEX_DEPLOYMENT=<auto-generated by Convex CLI>
NEXT_PUBLIC_CONVEX_URL=<your Convex cloud or local URL>
# Optional — defaults to NEXT_PUBLIC_CONVEX_URL if omitted
# NEXT_PUBLIC_CONVEX_SITE_URL=<Convex HTTP actions URL>
```

Then set the required Convex backend environment variables:
