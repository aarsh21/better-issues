...
if [ -n "$CONVEX_SELF_HOSTED_URL" ] && [ -n "$CONVEX_SELF_HOSTED_ADMIN_KEY" ]; then
  ...
  npx convex deploy $CONVEX_FLAGS 2>&1 || echo "!!! Convex deploy failed (backend may not be ready yet)"

  if [ -n "$BETTER_AUTH_SECRET" ]; then
    npx convex env set BETTER_AUTH_SECRET "$BETTER_AUTH_SECRET" $CONVEX_FLAGS 2>&1 || true
    echo "    BETTER_AUTH_SECRET set"
  fi
  if [ -n "$SITE_URL" ]; then
    npx convex env set SITE_URL "$SITE_URL" $CONVEX_FLAGS 2>&1 || true
    echo "    SITE_URL set"
  fi
