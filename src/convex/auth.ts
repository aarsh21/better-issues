...
const ATTACHMENT_UPLOAD_TOKEN_VERSION = 'v1';
...
const createAttachmentUploadSignature = async (
	signingSecret: string,
	userId: string,
	organizationId: string,
	issuedAt: number
	...
	return await signPayload(
		signingSecret,
		`attachment-upload:${userId}:${organizationId}:${issuedAt}`
	);
};
...
export const createAttachmentUploadToken = async ({
	...
	const signature = await createAttachmentUploadSignature(
		signingSecret,
		userId,
		organizationId,
		issuedAt
	);
	return `${ATTACHMENT_UPLOAD_TOKEN_VERSION}:${issuedAt}:${signature}`;
};
...
export const resolveAttachmentUploadToken = async ({
	...
	const [version, issuedAtRaw, signature, ...rest] = token.split(':');
	if (
		version !== ATTACHMENT_UPLOAD_TOKEN_VERSION ||
		typeof issuedAtRaw !== 'string' ||
		issuedAtRaw.length === 0 ||
		typeof signature !== 'string' ||
		signature.length === 0 ||
	...
	const expectedSignature = await createAttachmentUploadSignature(
		signingSecret,
		userId,
		organizationId,
		issuedAt
