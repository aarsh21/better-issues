<script lang="ts">
	...
	const fileUrls = useQuery(api.files.getUrls, () =>
		...
		try {
			const uploadResults = await Promise.all(
				filesToUpload.map(async (file) => {
					const upload = await generateUploadUrl({ organizationId });
...
<div class="grid gap-2">
	<Label for={inputId} class="text-sm">
		{field.label}
		{#if field.required}<span class="ml-1 text-destructive">*</span>{/if}
	</Label>
	...
	{:else if field.type === 'file'}
		<div class="grid gap-3">
			...
			{#if files.length === 0}
				<p class="text-xs text-muted-foreground">No files uploaded</p>
			{:else}
				<div class="grid gap-2">
