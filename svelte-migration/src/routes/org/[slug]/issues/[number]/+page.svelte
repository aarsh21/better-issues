<script lang="ts">
  ...
  const templateQuery = useQuery(
    api.templates.get,
    () => (issueQuery.data?.templateId ? { templateId: issueQuery.data.templateId } : "skip"),
  );
  ...
  const parsedSchema = $derived.by(() => {
    ...
    try {
      return JSON.parse(templateQuery.data.schema) as TemplateSchema;
    } catch {
      return null;
    }
  });
...
<div class="flex h-full flex-col">
  <div class="border-border flex items-center justify-between border-b px-4 py-3">
    <div class="flex items-center gap-3">
      ...
      {#if issue}
        <span class="text-muted-foreground font-mono text-sm">#{issue.number}</span>
      {:else}
  ...
  <div class="flex-1 overflow-auto">
    ...
    {:else}
      <div class="mx-auto grid max-w-3xl gap-8 p-6 md:grid-cols-[1fr_220px]">
          ...
          {:else}
            <div>
              ...
              {#if issue.description}
                <p class="text-muted-foreground mt-3 whitespace-pre-wrap text-sm">{issue.description}</p>
              {/if}
            </div>
          {/if}
          ...
          {#if parsedSchema && Object.keys(parsedTemplateData).length > 0}
            ...
            <div class="space-y-4">
              <p class="text-muted-foreground text-xs font-medium uppercase tracking-wider">
                {templateQuery.data?.name ?? "Template Data"}
              </p>
        ...
        <div class="space-y-6">
          <div class="space-y-3">
            <Label for="issue-status" class="text-muted-foreground text-xs uppercase tracking-wider">
              Status
            </Label>
            <div class="border-border flex items-center gap-2 border px-2 py-2">
              ...
              <select
                ...
                value={issue.status}
                onchange={(event) =>
                  void changeStatus((event.currentTarget as HTMLSelectElement).value as IssueStatus)}
              >
          ...
          <div class="space-y-3">
            <Label for="issue-priority" class="text-muted-foreground text-xs uppercase tracking-wider">
              Priority
            </Label>
            <div class="border-border flex items-center gap-2 border px-2 py-2">
          ...
          <div class="space-y-3">
            <Label class="text-muted-foreground text-xs uppercase tracking-wider">Labels</Label>
            <div class="flex flex-wrap gap-2">
              {#if labels.length === 0}
                <span class="text-muted-foreground text-xs">No labels configured</span>
              {:else}
