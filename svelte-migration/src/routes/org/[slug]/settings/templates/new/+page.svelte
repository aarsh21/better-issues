  ...
  <div class="flex-1 overflow-auto p-6">
    {#if preview}
      <div class="mx-auto max-w-lg space-y-6">
        <div>
          <h2 class="text-base font-medium">Preview: {name || "Untitled"}</h2>
          <p class="text-muted-foreground text-sm">{description || "No description"}</p>
        </div>
        <Separator />
        ...
        {#if previewSchema.fields.length > 0}
          <div class="space-y-4">
            {#each previewSchema.fields as field (field.key)}
              <TemplateFieldRenderer {field} readOnly value={undefined} organizationId={organizationId} />
            {/each}
          </div>
        {:else}
      ...
      <div class="mx-auto max-w-lg space-y-6">
        <div class="grid gap-4">
          <div class="grid gap-2">
            ...
            <Input id="template-name" placeholder="Bug Report" bind:value={name} />
          </div>
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <Label class="text-muted-foreground text-xs uppercase tracking-wider">Fields</Label>
          ...
          {#each fields as field, index (field.id)}
            <div class="border-border space-y-3 border p-3">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-1.5">
                  <GripVerticalIcon class="text-muted-foreground h-3.5 w-3.5" />
                  <span class="text-muted-foreground font-mono text-xs">Field {index + 1}</span>
                </div>
                <Button
                  type="button"
                  variant="ghost"
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="grid gap-1.5">
                  ...
                  <Input
                    ...
                    class="h-8 text-sm"
                    onchange={(event) =>
                      updateField(index, { label: (event.currentTarget as HTMLInputElement).value })}
                  />
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="grid gap-1.5">
                  ...
                  <select
                    ...
                    onchange={(event) =>
                      updateField(index, {
                        type: (event.currentTarget as HTMLSelectElement).value as TemplateField["type"],
                      })}
                  >
                ...
                <div class="flex items-end gap-2 pb-0.5">
                  <div class="flex items-center gap-2">
                    ...
                    />
                    <Label for={`required-${field.id}`} class="cursor-pointer text-xs">Required</Label>
                  </div>
                </div>
              </div>
              ...
              {#if field.type === "select"}
                <div class="grid gap-1.5">
                  <Label for={`field-options-${field.id}`} class="text-xs">Options (comma-separated)</Label>
                  <Input
              ...
              {#if field.type === "file"}
                <div class="grid gap-3">
                  ...
                  <div class="grid gap-1.5">
                    <Label for={`accept-${field.id}`} class="text-xs">Accepted file types (optional)</Label>
                    <Input
                      id={`accept-${field.id}`}
              ...
              {:else}
                <div class="grid gap-1.5">
                  <Label for={`placeholder-${field.id}`} class="text-xs">Placeholder (optional)</Label>
                  <Input
        ...
        <div class="space-y-2">
          <Label class="text-muted-foreground text-xs uppercase tracking-wider">JSON Schema</Label>
          <pre class="border-border bg-muted overflow-auto border p-3 text-xs font-mono">{jsonPreview}</pre>
        </div>
